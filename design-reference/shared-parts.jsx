...
// ───────── Home AI Showcase section (Featured Grid) ─────────
function HomeShowcase() {
  const items = (typeof PROJECTS !== "undefined" ? PROJECTS.filter(p => p.featured).slice(0, 3) : []);
  const Heart = () => <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M20.8 4.6a5.5 5.5 0 0 0-7.8 0L12 5.7l-1-1a5.5 5.5 0 0 0-7.8 7.8l1 1L12 21l7.8-7.6 1-1a5.5 5.5 0 0 0 0-7.8Z"/></svg>;
  const Eye = () => <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M2 12s3.5-7 10-7 10 7 10 7-3.5 7-10 7-10-7-10-7Z"/><circle cx="12" cy="12" r="3"/></svg>;
  const Cmt = () => <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 11.5a8.4 8.4 0 0 1-9 8.4 9 9 0 0 1-4-.9L3 21l1-4a8.4 8.4 0 0 1-1-4 8.5 8.5 0 0 1 9-8.4 8.5 8.5 0 0 1 8 8.4Z"/></svg>;

  return (
    <div className="sec" style={{ padding: "30px 22px 24px" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", flexWrap: "wrap", gap: 16, marginBottom: 22 }}>
        <div>
          <div style={{ fontSize: 30, fontWeight: 800, letterSpacing: "-.025em", lineHeight: 1.06, margin: "0", color: "#1a1325" }}>
            Built by the community, <span style={{ fontStyle: "italic", color: "#7c3aed" }}>powered by AI.</span>
          </div>
          <div style={{ fontSize: 15, color: "#5a4e75", lineHeight: 1.55, marginTop: 10, maxWidth: "56ch" }}>
            See what people are building with the models you're learning — then ship your own and get it in front of everyone.
          </div>
        </div>
        <span className="feat-cta" onClick={() => { window.location.href = "AI Showcase Page Wireframes.html"; }}
          style={{ display: "inline-flex", alignItems: "center", gap: 8, border: "1.5px solid #d9cff0", background: "#fff", color: "#7c3aed", fontWeight: 700, fontSize: 14, padding: "11px 18px", borderRadius: 11, cursor: "pointer", whiteSpace: "nowrap", transition: "all .15s ease" }}
          onMouseEnter={(e) => { e.currentTarget.style.borderColor = "#7c3aed"; e.currentTarget.style.background = "#ece6f7"; }}
          onMouseLeave={(e) => { e.currentTarget.style.borderColor = "#d9cff0"; e.currentTarget.style.background = "#fff"; }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 18 }}>
        {items.map(p => (
          <div key={p.id} onClick={() => { window.location.href = "AI Showcase Page Wireframes.html"; }}
            style={{ background: "#fff", border: "1.5px solid #d9cff0", borderRadius: 16, overflow: "hidden", cursor: "pointer", transition: "transform .2s ease, box-shadow .2s ease, border-color .2s ease" }}
            ...
            <div style={{ position: "relative", height: 158, display: "grid", placeItems: "center", background: p.grad }}>
              <div style={{ fontFamily: "var(--mono)", fontWeight: 800, fontSize: 38, color: "rgba(255,255,255,.92)", textShadow: "0 2px 20px rgba(0,0,0,.25)" }}>{p.mono}</div>
              <div style={{ position: "absolute", top: 11, left: 11, right: 11, display: "flex", alignItems: "center", gap: 8 }}>
                <span style={{ display: "inline-flex", alignItems: "center", gap: 6, background: "rgba(255,255,255,.92)", borderRadius: 999, padding: "5px 11px", fontFamily: "var(--mono)", fontSize: 11.5, fontWeight: 700, color: "#1a1325" }}><Eye /> {p.views}</span>
                {p.featured && <span style={{ marginLeft: "auto", background: "#7c3aed", color: "#fff", borderRadius: 999, padding: "5px 11px", fontSize: 10, fontWeight: 800, letterSpacing: ".04em", textTransform: "uppercase" }}>Featured</span>}
              </div>
            </div>
            <div style={{ padding: "14px 15px 15px" }}>
              <div style={{ fontSize: 15, fontWeight: 800, letterSpacing: "-.01em", lineHeight: 1.25 }}>{p.title}</div>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 9, fontSize: 12, color: "#5a4e75", fontWeight: 600 }}>
                <span style={{ width: 22, height: 22, borderRadius: "50%", display: "grid", placeItems: "center", color: "#fff", fontWeight: 700, fontSize: 9, background: p.author.color }}>{p.author.initials}</span>
                {p.author.name}
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 14, marginTop: 11, paddingTop: 11, borderTop: "1px solid #d9cff0", fontSize: 12, color: "#5a4e75", fontWeight: 600 }}>
                <span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}><span style={{ color: "#9087a8" }}><Heart /></span> {p.likes}</span>
                <span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}><span style={{ color: "#9087a8" }}><Cmt /></span> {p.comments.length}</span>
                <span style={{ marginLeft: "auto" }}><span style={{ width: 22, height: 22, border: "1.5px solid #7c3aed", borderRadius: 7, display: "grid", placeItems: "center", background: "#ece6f7", overflow: "hidden" }}><MarkGlyph mark={p.mark} /></span></span>
              </div>
            </div>
          </div>
        ))}
  ...
  return (
    <div className="ftr" style={{ borderRadius: 6, margin: "0 0 0 0" }}>
      <div className="ftr-grid">
        ...
        <div className="ftr-col">
          <h6>Company</h6>
          <ul><li>About</li><li>Newswire</li><li>Careers</li><li>Contact</li><li><a href="Terms of Service Page Wireframes.html" style={{ color:"inherit", textDecoration:"none" }}>Terms of Service</a></li><li><a href="Privacy Policy Page Wireframes.html" style={{ color:"inherit", textDecoration:"none" }}>Privacy Policy</a></li></ul>
        </div>
        <div className="ftr-col">
          <h6>Newsletter</h6>
...
Object.assign(window, { NavStrip, AiButton, AiGrid2x4, FeatureSection, HomeShowcase, NewsSection, NewsImage, Footer, LearnAiArt, EcosystemArt, SectionDivider });
